...
class CrawlerBNC{
	...
	function getTopic1($url){ 
		echo '<pre>';
		var_dump($this->getListBySelecter('.type-post',$url));
	}

	function getListBySelecter($selector,$url){
		$html = '<?xml encoding="UTF-8">'.$this->getContentUrl($url); 
		$node = $this->select_elements($selector,$html,0);
		$r = array();
		foreach ($node as $key => $value) {
			$h = '<?xml encoding="UTF-8">'. $this->buildOutputFromDOMNode($value);
			$v['link'] = $this->select_elements('.entry-content > a',$h)[0]['attributes']['href'];
			$v['title'] = $this->select_elements('.post-title > a',$h)[0]['text'];
			$v['brief'] = $this->select_elements('.post-excerpt',$h)[0]['text'];
			$v['image'] = $this->select_elements('.entry-content > a > img',$h)[0]['attributes']['src'];
			$v['content'] = $this->getContentDetail($v['link']);
			$r[] = $v;
		}
		return $r;
	}
	function getContentDetail($url){
		$html = '<?xml encoding="UTF-8">'.$this->getContentUrl($url); 
		$node = $this->select_elements('#entry-content',$html,0)[0];

		return $this->buildOutputFromDOMNode($node);
	}
